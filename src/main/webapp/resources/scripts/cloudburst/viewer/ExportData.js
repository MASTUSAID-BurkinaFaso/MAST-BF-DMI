...
Cloudburst.ExportData = function (map, _searchdiv) {
   ...
   // removeChildElement("sidebar", "layerSwitcherContent");
    ...
    jQuery.get('resources/templates/viewer/export.html', function (template) {
		...
		addTab($._('home_page_exportdata'),template);
        
		$("#export-help").tipTip({defaultPosition:"right"});
		
		translateQueryStrings1();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#formats').append($("<option></option>").attr("value", "").text("Please Select"));
                	...
                	$('#formats').append($("<option></option>").attr("value", "").text("Merci de sélectionner"));
...
function translateQueryStrings1(){
	
	$('#exprt_selectlayer').html($._('Selected_Layer'));
	$('#options-s-t').html($._('Export_Options'));
	$('#all1').html($._('Feature'));
	$('#selected1').html($._('Selected_feature'));
	$('#selectRegion1').html($._('Select_region'));
	$('#export_format').html($._('Format'));
	$('#export_msg').html($._('Export_msg'));
	$('#btnDownload').attr("value",$._('export_download'));
}
