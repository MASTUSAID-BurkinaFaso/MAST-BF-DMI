...
var CONST_SELECT_LAYER_FR = "Sélectionner la couche";
var CONST_SELECT_QUERY_FR = "Requête de selection";
//Amitabh (Temporary)
var layerMap = [];
var blnRelate = false;
var geomFieldName;
...
Cloudburst.Query = function(_showResultsinDialog) {
	...
	jQuery.get('resources/templates/viewer/NewQueryBuilder.html', function(template) {
		...
		//Add tad
		addTab($._('home_page_querybuilder'),template);
...
function translateQueryStrings(){
	$('#qrybldr_selectlayer').html($._('Select_Layer'));
	$('#qry_fields').html($._('Fields'));
	$('#qry_operators').html($._('Operators'));
	$('#qry_values').html($._('Values'));
	$('#displayValues').html($._('More'));
	$('#qry_loadquery').html($._('Load_Query'));
	$('#qry_whereclause').html($._('Where_Clause'));
	$('#saveQueryLnk').html($._('Save_Query'));
	$('#btnClear').html($._('Clear'));
	$('#lblqryName').html($._('Query_Name'));
	$('#qryDesc').html($._('Query_Desc'));
	$('#btnSaveQry').attr("value", $._('Save_Query'));
	$('#btnRunQry').attr("value", $._('Run_Query'));
}
...
function populateFeatureLayers() {
    ...
    $('#relateLayer').append($("<option></option>").attr("value", CONST_SELECT_LAYER).text(CONST_SELECT_LAYER));
    	 ...
    	 $('#layers').append($("<option></option>").attr("value", CONST_SELECT_LAYER).text(CONST_SELECT_LAYER_FR));
    	    $('#relateLayer').append($("<option></option>").attr("value", CONST_SELECT_LAYER).text(CONST_SELECT_LAYER_FR));
    }
        ...
        $.ajax({
            ...
            error: function (xhr, status) {
            	if(selectedLayer.indexOf("OSMM_") > -1){
            		jAlert(WFS_oepration1 + 'OSMasterMap'+ WFS_oepration2);
            	}else{
            		jAlert(FAILED);
            	}
            }
...
function populateFields(_data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#fields_1').append($("<option></option>").attr("value", "").text(Select_field));
        ...
        error: function (xhr, status) {
        	if(selectedLayer.indexOf("OSMM_") > -1){
        		...
        		jAlert(WFS_oepration1 + selectedLayer + WFS_oepration2);
        	}else{
        		jAlert(FAILED);
        	}
        }
...
function parseFeatures(features, selectedField) {
    ...
    if(lang=="en")
    $('#values').append($("<option></option>").attr("value", "").text("Please select"));
    else if (lang=="fr")
    	$('#values').append($("<option></option>").attr("value", "").text("Merci de sélectionner"));
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#selectQuery').append($("<option></option>").attr("value", CONST_SELECT_QUERY).text(CONST_SELECT_QUERY));
        		 ...
        		 $('#selectQuery').append($("<option></option>").attr("value", CONST_SELECT_QUERY).text(CONST_SELECT_QUERY_FR));
...
function saveQuery(){
	...
	if(name!="" && whereClause!=""){
		...
		jAlert("Please enter query name");
			...
			jAlert("S'il vous plaît entrez le nom de la requête");
	}
