...
Cloudburst.Editing = function (_map, _searchdiv) {    
    ...
    jQuery.get('resources/templates/viewer/editing.html', function (template) {
      ...
      //Add tad
		...
		addTab($._('home_page_editing'),template);
		$('#selectionBox').attr("title",$._('home_page_selectbyrectangle'));
		$('#clearselection').attr("title",$._('home_page_clear'));
		$('#selectUndo').attr("title",$._('editing_undo'));
		$('#selectRedo').attr("title",$._('editing_redo'));
		$('#selectSave').attr("title",$._('editing_save'));
		$('#point').attr("title",$._('editing_point'));
		$('#line').attr("title",$._('editing_line'));
		$('#polygon').attr("title",$._('editing_polygon'));
		$('#importFeature').attr("title",$._('editing_import'));
		$('#move').attr("title",$._('editing_move'));
		$('#resize').attr("title",$._('editing_resize'));
		$('#reshape').attr("title",$._('editing_reshape'));
		$('#rotate').attr("title",$._('editing_rotate'));
		$('#removeFeature').attr("title",$._('editing_removeFeature'));
		$('#editAttribute').attr("title",$._('editing_editAttribute'));
		$('#options2-s-t').html($._('editing_editAttribute'));
		$('#editor_apply').html($._('editing_apply'));
		$('#tolerence_id').html($._('editing_tolerence'));
		$('#maxfeatures_id').html($._('editing_maxfeatures'));
		$('#options4-s-t').html($._('editing_snapsetting'));
		$('#snap_feature').html($._('editing_snap_feature'));
		$('#snap_node').html($._('editing_snap_node'));
		$('#snap_vertice').html($._('editing_snap_vertice'));
		$('#snap_edge').html($._('editing_snap_edge'));
		$('#lbl_edit_layer').html($._('editing_layer'));
			...
			if($('#edit_layer').val()!=""){
				...
				case 'point':
					if(layerType == 'Point'){
						...
						("Layer selected is not point type", "Error");
					}
					break;
				case 'line':
...
function populateEditableLayers() {
	...
	$('#edit_layer').append($("<option></option>").attr("value", "").text("Select Layer"));
    	...
    	$('#edit_layer').append($("<option></option>").attr("value", "").text("Merci de sélectionner"));
