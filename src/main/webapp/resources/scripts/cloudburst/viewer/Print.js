...
Cloudburst.Print = function(_map, _searchdiv) {
	...
	jQuery.get('resources/templates/viewer/print.html', function(
		...
		addTab($._("home_page_print"), template);

		$("#print-help").tipTip({
			defaultPosition : "right"
		});
		$('#map_title').html($._('print_title'));
		$('#btnPrint').attr("value", $._('home_page_print'))
...
var printChoice = function(){
	...
	$("#print_ok").html('<span class="ui-button-text">'+ Ok +'</span>');
	$("#print_cancel").html('<span class="ui-button-text">'+ Cancel +'</span>');
	Dialog.dialog("open");
}
	...
	jQuery.ajax(
			{
				...
				success: function (data1) 
				{
					...
					var layerurl_ccro = wmsurl +"bbox="+bbox_temp+"&FORMAT=image/png&REQUEST=GetMap&layers="+printlayer+"&width=600&height=600&srs=EPSG:4326&cql_Filter=INCLUDE;usin%20in("+arr.toString()+")";
