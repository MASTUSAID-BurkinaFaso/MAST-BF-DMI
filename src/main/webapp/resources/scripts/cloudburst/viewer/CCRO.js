	...
	jQuery.ajax(
			{
				...
				success: function (data1) 
				{
					...
					//Tenancy in Minor(Guardian)
						...
						//remove bold from "na"
						var highlight = " na ";  
					    var span = '<span style="font-weight:normal">' + highlight + '</span>';
					    var text = $('#_idguardian').html();
					    $('#_idguardian').html(text.replace(highlight, span));
					...
					$("#_surveyYear").text(dtoList.surveyYear);
					
					$('._idowner').text(owner);
					$('._idAddress').text(dtoList.address);
...

					{
						...
						if(dtoList.deceasedPerson!=null && dtoList.deceasedPerson.toString()!="")
							var deceasedPerson =arraytoname(dtoList.deceasedPerson);
						
						if(dtoList.adminName.length==1){

							$('#single_owner').text("Msimamizi(Mkazi)");
							$('#_idDeceased').text(deceasedPerson);
							$('._idowner').text(admin);
					...
					if (dtoList.adminName.length!=0 && dtoList.adminName!=null){
						//var array_owner=[];
						for (var int = 0; int <dtoList.adminName.length; int++) {
							var name=dtoList.adminName[int];
							var result_element = dtoList.admin_url[name];
							//array_owner.push(name);
							//	$( "#singlepicture" ).insertAfter( ".owner" );
							jQuery("#documentId-div").append('<div class="row-fluid"><div class="span8"><p></p>'+
									'<p> <span class="left-50-alt">Jina </span> <span class="left-50">Saini/dole gumba </span></p>'+'<p><br><br><br></p>'+
									'<p><span id="occupier_adminname'+""+int+""+'" class="left-50-alt"> </span> <span	class="left-50">.................... </span></p></div>'+
									'<div class="span4"><p class="type">PICHA</p><p class="type"><img id="_adminImage'+""+int+""+'" /></p></div></div>');
							//jQuery("#documentId-div").append('<td><div class="fieldHolder"><div class="floatColumn02"><img id="_occuiperImage'+""+int+""+'" /></div></div></td>');

							jQuery('#_adminImage'+""+int+""+'').attr("src","http://"+location.host+"/"+result_element);
							jQuery('#occupier_adminname'+""+int+""+'').text(name);





							jQuery('#_adminImage'+""+int+""+'').attr("height",126);
							jQuery('#_adminImage'+""+int+""+'').attr("width",105);
					...
					//remove bold from "na"
						var highlight = " na ";  
					    var span = '<span style="font-weight:normal">' + highlight + '</span>';
					    var text = $('._idowner').html();
					    $('._idowner').html(text.replace(highlight, span));
