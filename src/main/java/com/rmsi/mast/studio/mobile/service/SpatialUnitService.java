...
public interface SpatialUnitService {
	...
	NatureOfPower getNop(int nopId);

	Village getVillageById(int villageId);

	NatureOfApplication getNoaById(int noaId);

	MutationType getMtById(int mtId);

	ParcelType getParcelTypeById(int parcelId);

	TitleExisting getExistingTitleById(int parseInt);
	
	Integer getExistingId(int landUseTypeId);
